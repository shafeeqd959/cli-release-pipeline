...
const { cliux: cliUX, messageHandler } = require("testsha-utilities");
...
const exportBasePath = path.join(__dirname, "..", "..", EXPORT_PATH);
      ...
      .it("Check locale count is done", async () => {
        ...
        try {
          if (fs.existsSync(localeJson)) {
            exportedLocaleCount = Object.keys(
              JSON.parse(fs.readFileSync(localeJson, "utf-8"))
            ).length;
          }
        } catch (error) {
          console.trace(error);

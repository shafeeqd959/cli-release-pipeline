...
exports.validateConfig = function (importConfig) {
  if (
    ...
    addlogs(
      importConfig,
      chalk.red("Kindly provide management_token or email and password"),
      "error"
    );
    return "error";
  } else if (
    ...
    addlogs(
      importConfig,
      chalk.red("Kindly provide Email and password for old version stack"),
      "error"
    );
    return "error";
  } else if (
    ...
    addlogs(
      importConfig,
      chalk.red("Kindly provide Email and password"),
      "error"
    );
    return "error";
  }
...
exports.sanitizeStack = function (importConfig) {
  if (
    typeof importConfig.preserveStackVersion !== "boolean" ||
    !importConfig.preserveStackVersion
  ) {
  ...
  try {
    ...
    return httpClient
      .get(`https://${importConfig.host}/v3${importConfig.apis.stacks}`)
      .then((stackDetails) => {
        if (
          stackDetails.data &&
          stackDetails.data.stack &&
          stackDetails.data.stack.settings
        ) {
          ...
          if (
            !oldStackDetails ||
            !oldStackDetails.settings ||
            !oldStackDetails.settings.hasOwnProperty("version")
          ) {
            throw new Error(`${JSON.stringify(oldStackDetails)} is invalid!`);
          }
          const oldStackDate = new Date(
            oldStackDetails.settings.version
          ).toString();
          ...
          if (oldStackDate > newStackDate) {
            ...
            addlogs(
              importConfig,
              "The version of both the stacks are same.",
              "success"
            );
            return Promise.resolve();
          }
          ...
          return httpClient
            .put(
              `https://${importConfig.host}/v3${importConfig.apis.stacks}settings/set-version`,
              {
                stack_settings: {
            ...
            .then((response) => {
              addlogs(
                importConfig,
                `Stack version preserved successfully!\n${JSON.stringify(
                  response.data
                )}`,
                "success"
              );
        ...
        throw new Error(
          `Unexpected stack details ${
            stackDetails && JSON.stringify(stackDetails.data)
          }`
        );
      });
  } catch (error) {
...
exports.masterLocalDetails = function (credentialConfig) {
  ...
  return new Promise((resolve, reject) => {
    var result = client
      .stack({
        api_key: credentialConfig.target_stack,
        management_token: credentialConfig.management_token,
      })
      .locale()
      .query();
    result
...
exports.field_rules_update = function (importConfig, ctPath) {
  return new Promise(function (resolve, reject) {
    ...
    fs.readFileSync(
      path.join(ctPath + "/field_rules_uid.json"),
      async (err, data) => {
        if (err) {
          throw err;
        }
        ...
        if (ct_field_visibility_uid && ct_field_visibility_uid != "undefined") {
          for (let index = 0; index < ct_field_visibility_uid.length; index++) {
            if (
              ct_files.indexOf(ct_field_visibility_uid[index] + ".json") > -1
            ) {
              let schema = require(path.resolve(
                ctPath,
                ct_field_visibility_uid[index]
              ));
              // await field_rules_update(schema)
              let fieldRuleLength = schema.field_rules.length;
              for (let k = 0; k < fieldRuleLength; k++) {
                let fieldRuleConditionLength =
                  schema.field_rules[k].conditions.length;
                for (let i = 0; i < fieldRuleConditionLength; i++) {
                  if (
                    schema.field_rules[k].conditions[i].operand_field ===
                    "reference"
                  ) {
                    ...
                    let entryUidMapperPath = path.join(
                      entryMapperPath,
                      "uid-mapping.json"
                    );
                    let fieldRulesValue =
                      schema.field_rules[k].conditions[i].value;
                    let fieldRulesArray = fieldRulesValue.split(".");
                    let updatedValue = [];
                    for (let j = 0; j < fieldRulesArray.length; j++) {
                      let splitedFieldRulesValue = fieldRulesArray[j];
                      let oldUid = helper.readFileSync(
                        path.join(entryUidMapperPath)
                      );

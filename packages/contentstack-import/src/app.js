  ...
  return new Promise(async function (resolve, reject) {
    ...
    const backupAndImportData = async () => {
      if (fs.existsSync(config.data)) {
        let migrationBackupDirPath = path.join(
          process.cwd(),
        ...
        return createBackup(migrationBackupDirPath, config)
          ...
          .catch((error) => {
            addlogs(
              config,
              `Failed to import contents ${formatError(error)}`,
              "error"
            );
            reject(e);
            process.exit(1);
        ...
        addlogs(
          config,
          chalk.red(filename + " Folder does not Exist"),
          "error"
        );
      }
    };
    ...
    if (types.indexOf(moduleName) > -1) {
      if (!config.master_locale) {
        try {
          ...
          addlogs(
            config,
            `Failed to get master locale detail from the stack ${formatError(
              error
            )}`,
            "error"
          );
      ...
      const importResponse = await new ImportModule(config, stackClient).start(
        config
      );
      ...
      addlogs(
        ...
        "The log for this is stored at " +
          path.join(config.oldPath, "logs", "import"),
        "success"
      );
      return true;
    } else {
  ...
  try {
    ...
    if (config.target_stack && config.source_stack) {
      ...
      addlogs(
        config,
        "The log for this is stored at " +
          path.join(config.data, "logs", "import"),
        "success"
      );
    } else {
      addlogs(
        config,
        chalk.green(
          "Stack: " + config.target_stack + " has been imported succesfully!"
        ),
        "success"
      );
      addlogs(
...
function createBackup(backupDirPath, config) {
  return new Promise((resolve, reject) => {
    if (
      config.hasOwnProperty("useBackedupDir") &&
      fs.existsSync(config.useBackedupDir)
    ) {
    ...
    const result = await httpClient
      ...
      .then(({ data }) => {
        if (data.error_message) {
          ...
          addlogs(
            config,
            chalk.red("No branch found with the name " + branch),
            "error"
          );
          reject();
        }

...
class CrossPublishCommand extends Command {
  async run() {
    const crossPublishFlags = this.flagsAdapter(
      this.parse(CrossPublishCommand).flags
    );
    let updatedFlags;
    try {
      updatedFlags = crossPublishFlags.config
        ? store.updateMissing(configKey, crossPublishFlags)
        : crossPublishFlags;
    } catch (error) {
...
CrossPublishCommand.flags = {
  ...
  retryFailed: flags.string({
    char: "r",
    description:
      "(optional) Retry publishing failed entries from the logfile (this flag overrides all other flags)",
    hidden: true,

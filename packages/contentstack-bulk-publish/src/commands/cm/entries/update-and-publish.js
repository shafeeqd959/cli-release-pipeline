...
class UpdateAndPublishCommand extends Command {
  async run() {
    ...
    addFieldsFlags.retryFailed =
      addFieldsFlags["retry-failed"] || addFieldsFlags.retryFailed || false;
    addFieldsFlags.contentTypes =
      addFieldsFlags["content-types"] || addFieldsFlags.contentTypes;
    addFieldsFlags.bulkPublish =
      addFieldsFlags["bulk-publish"] || addFieldsFlags.bulkPublish;
    ...
    let updatedFlags;
    try {
      updatedFlags = addFieldsFlags.config
        ? store.updateMissing(configKey, addFieldsFlags)
        : addFieldsFlags;
    } catch (error) {
...
UpdateAndPublishCommand.flags = {
  ...
  force: flags.boolean({
    default: false,
    description:
      "Update and publish all entries even if no fields have been added",
  }),
};

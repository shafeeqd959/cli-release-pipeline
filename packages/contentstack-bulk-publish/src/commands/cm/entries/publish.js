...
class PublishEntriesCommand extends Command {
  async run() {
    ...
    entriesFlags.retryFailed =
      entriesFlags["retry-failed"] || entriesFlags.retryFailed || false;
    entriesFlags.contentTypes =
      entriesFlags["content-types"] || entriesFlags.contentTypes;
    entriesFlags.bulkPublish =
      entriesFlags["bulk-publish"] || entriesFlags.bulkPublish;
    entriesFlags.publishAllContentTypes =
      entriesFlags["publish-all-content-types"] ||
      entriesFlags.publishAllContentTypes ||
      false;
    ...
    let updatedFlags;
    try {
      const storeConfigKey = entriesFlags["source-env"]
        ? configKeyCrossEnv
        : configKey;
      updatedFlags = entriesFlags.config
        ? store.updateMissing(storeConfigKey, entriesFlags)
        : entriesFlags;
    } catch (error) {
      ...
      if (!updatedFlags.retryFailed) {
        if (!updatedFlags.alias) {
          updatedFlags.alias = await cliux.prompt(
            "Provide the alias of the management token to use"
          );
        }
...
PublishEntriesCommand.flags = {
  ...
  "publish-all-content-types": flags.boolean({
    description:
      "(optional) Publish all contenttypes (cannot be set when contentTypes flag is set)",
  }),
  publishAllContentTypes: flags.boolean({
    char: "o",
    description:
      "Publish all content-types (optional, cannot be set when contentTypes flag is set)",
    hidden: true,
    parse: printFlagDeprecation(
      ["-o", "--publishAllContentTypes"],

    ...
    stack
      ...
      .then((allEnvironments) => {
        const filteredEnvironments = allEnvironments.items
          .filter(
            (environment) => environments.indexOf(environment.name) !== -1
          )
      ...
      elements[loc].forEach((entry) => {
        if (entry.publish_details.length > 0) {
          entry.publish_details = entry.publish_details.filter(
            (element) =>
              environments.indexOf(element.environment) !== -1 &&
              element.locale === loc
          );
        }
      });
...
async function formatLogData(stack, data) {
  ...
  switch (type) {
    case "entry":
      ...
      for (let i = 0; i < data.length; i += 1) {
        if (
          formattedLogs.locale.indexOf(data[i].message.options.locale) === -1
        ) {
        ...
        if (data[i].message.options.entries) {
          ...
          formattedLogs.entries[data[i].message.options.locale] =
            formattedLogs.entries[data[i].message.options.locale].concat(
              data[i].message.options.entries
            );
        } else {
        ...
        if (!formattedLogs.environments)
          formattedLogs.environments = data[i].message.options.environments;
        if (!formattedLogs.api_key)
          formattedLogs.api_key = data[i].message.api_key;
      }
      break;
    case "asset":
      ...
      for (let i = 0; i < data.length; i += 1) {
        if (data[i].message.options.assets) {
          ...
          formattedLogs.assets = formattedLogs.assets.concat(
            data[i].message.options.assets
          );
        } else {
  ...
  formattedLogs.environments = await getEnvironmentUids(
    stack,
    formattedLogs.environments
  );
...
async function revertUsingLogs(logFileName) {
  ...
  if (validateFile(logFileName)) {
    ...
    if (response.file.length > 0) {
        ...
        switch (logs.type) {
            ...
            logs.locale.forEach((loc, locIndex) => {
              logs.entries[loc].forEach(
                (
                  ...
                  const publishDetailsForThisEnvironment =
                    publish_details.filter(
                      (publishDetail) =>
                        publishDetail.environment === environment.uid
                    );

                  if (publishDetailsForThisEnvironment.length > 0) {
                    // handle revert case

                    publishDetailsForThisEnvironment.forEach(
                      (publishDetail) => {
                  ...
                  if (entryIndex === logs.entries[loc].length - 1) {
                    if (
                      bulkUnpublishSet.length <= 10 &&
                      bulkUnpublishSet.length !== 0
                    ) {
                      unpublishQueue.Enqueue({
                        entries: bulkUnpublishSet,
                        environments: [environment.name],
                  ...
                  if (
                    ...
                    mapSeries(
                      setOfBulkPublishRequestPayloads,
          ...
          case "asset":
            logs.assets.forEach(({ publish_details, uid }, assetIndex) => {
              ...
              if (
                envIndex === logs.environments.length - 1 &&
                assetIndex === logs.assets.length - 1
              ) {
                mapSeries(
                  setOfBulkPublishRequestPayloads,
                  processPublishRequests
      ...
      console.log(
        chalk.red(
          "Error: This log file is empty. Please check error logs if any"
        )
      );
    }
  }

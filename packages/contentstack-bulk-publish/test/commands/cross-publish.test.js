...
const { cliux, configHandler } = require("testsha-utilities");
...
describe("cross-publish", () => {
  ...
  beforeEach(() => {
    ...
    nock(dummyConfig.get("cdnEndPoint"), {
      ...
      .post(
        /\/content_types\/dummyContentType\/entries\/([a-zA-Z0-9]*)\/publish/
      )
  ...
  test
    ...
    .catch((error) => {
      expect(error.message).to.contain(
        "Environment, Destination Environment, ContentType is required for processing this command. Please check --help for more details"
      );
    })
    .it("runs hello");
    ...
    .stub(require("../../src/util/queue.js"), "getQueue", () => {
      return {
        Enqueue: function (data) {
          if (data.assets)
            return console.log(
              `${data.assets.length} elements of type ${data.Type} has been queued to be published`
            );
          if (data.entries)
            return console.log(
              `${data.entries.length} elements of type ${data.Type} has been queued to be published`
            );
          return console.log(
            `Data of type ${data.Type} has been queued to be published`
          );
        },
      };
    })
      ...
      return {
        Enqueue: function (data) {
          if (data.bulkPublishAssetSet)
            return console.log(
              `${data.bulkPublishAssetSet.length} elements of type ${data.Type} has been queued to be published`
            );
          if (data.bulkPublishSet)
            return console.log(
              `${data.bulkPublishSet.length} elements of type ${data.Type} has been queued to be published`
            );
          return console.log(
            `Data of type ${data.Type} has been queued to be published`
          );

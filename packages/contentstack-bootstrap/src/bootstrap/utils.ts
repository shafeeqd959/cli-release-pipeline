...
export const setupEnvironments = async (
  ...
  const environmentResult = await managementAPIClient
    ...
    .find();
  if (
    Array.isArray(environmentResult.items) &&
    environmentResult.items.length > 0
  ) {
    for (const environment of environmentResult.items) {
      if (environment.name) {
        ...
        try {
          ...
          if (tokenResult.token) {
            ...
            cliux.print(
              messageHandler.parse(
                "CLI_BOOTSTRAP_APP_FAILED_TO_CREATE_TOKEN_FOR_ENV",
                environment.name
              )
            );
          }
          ...
          cliux.print(
            messageHandler.parse(
              "CLI_BOOTSTRAP_APP_FAILED_TO_CREATE_ENV_FILE_FOR_ENV",
              environment.name
            )
          );
        }
    ...
    cliux.error(
      messageHandler.parse("CLI_BOOTSTRAP_APP_ENV_NOT_FOUND_FOR_THE_STACK")
    );
  }
};
  ...
  switch (appConfigKey) {
    ...
    case "angular":
      ...
      fileName = `environment${
        environmentVariables.environment === "production" ? ".prod." : "."
      }ts`;
    ...
    case "vue-starter":
      fileName = ".env";
      filePath = path.join(clonedDirectory, fileName);
      content = `VUE_APP_CONTENTSTACK_API_KEY=${
        environmentVariables.api_key
